...
final class HabitManagementUITests: XCTestCase {
    ...
    func testCompleteFirstHabitOwnerJourney() {
        ...
        XCTAssertTrue(app.buttons["Plant a habit"].exists)
        recordScreenshot(named: "First launch Today", of: app)

        app.buttons["Plant a habit"].tap()
        ...
        nameField.tap()
        targetField.tap()
        XCTAssertTrue(app.staticTexts["Enter a habit name."].waitForExistence(timeout: 2))
        ...
        nameField.tap()
        XCTAssertTrue(app.staticTexts["Enter a unit."].waitForExistence(timeout: 2))
        XCTAssertFalse(saveButton.isEnabled)
        ...
        XCTAssertTrue(app.staticTexts[
            "Set at creation. To change cadence, archive this habit and plant a new one."
        ].exists)
        ...
        let weeklyConsequence = "Deleting Write lasting field notes also removes 2 activity periods, 1 bucket, and 0 log entries. This can't be undone."
