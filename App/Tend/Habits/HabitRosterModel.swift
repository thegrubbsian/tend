...
struct HabitRosterDeletionConfirmation: Identifiable {
  ...
  var consequenceText: String {
    ...
    return
      "\(archivedState)Deleting \(name) also removes \(activityPeriods), \(buckets), and \(entries). This can't be undone."
  }
...
final class HabitRosterModel {
  ...
  func retryOperation(
    ...
    let dismissesDeletionConfirmation =
      retryRequest.kind == .archive
...
extension HabitRosterModel {
  fileprivate struct RefreshContext {
    let instant: Date
  ...
  fileprivate struct MutationRequest {
    enum Kind: Equatable {
      case archive
      case reactivate
      case delete
