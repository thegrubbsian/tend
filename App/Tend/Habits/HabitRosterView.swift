...
struct HabitRosterView: View {
    ...
    var body: some View {
        ...
        .sheet(item: $presentedForm, onDismiss: {
            refresh(at: .now)
        }) { form in
        ...
        if !rows.isEmpty {
            ...
            ForEach(rows) { row in
                HabitRosterRowCard(row: row, retryStreak: model.retryRefresh)
                    .modifier(HabitRosterActionsModifier(
                        row: row,
...
private struct HabitRosterSectionHeaderModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            .modifier(HabitRosterListRowModifier())
            .padding(.top, hasPrecedingSection ? AlmanacMetrics.spacingLarge : AlmanacMetrics.spacingSmall)
