import CoreGraphics

enum AlmanacMetrics {
  static let spacingSmall: CGFloat = 8
  static let spacingMedium: CGFloat = 16
  static let spacingLarge: CGFloat = 24
  static let spacingExtraLarge: CGFloat = 40

  static let screenPadding: CGFloat = 20
  static let readableContentWidth: CGFloat = 600

  static let cardRadius: CGFloat = 14
  static let insetRadius: CGFloat = 10
  static let gardenCellRadius: CGFloat = 7
  static let gardenOutlineWidth: CGFloat = 1.5
  static let tabPillRadius: CGFloat = 36

  static let tabPillHeight: CGFloat = 62
  static let tabPillMaximumWidth: CGFloat = 360
  static let tabPillEdgeInset: CGFloat = 21
  static let tabPillInset: CGFloat = 4
  static let tabItemHeight: CGFloat = 54
  static let tabIconSize: CGFloat = 18
  static let minimumTarget: CGFloat = 44
}
